...
const Menu = ({ className, items }: MenuProps) => (
  <div className={className}>
    {items.map((item, index) => (
      <Link
        className="group w-[90%] flex items-center justify-between mb-4 p-8 border border-n-3 rounded-xl  transition-all hover:border-transparent hover:shadow-[0_0_1rem_0.25rem_rgba(0,0,0,0.04),0px_2rem_1.5rem_-1rem_rgba(0,0,0,0.12)] last:mb-0 2xl:p-2.5 lg:p-3.5 dark:border-n-5 dark:hover:border-n-7 dark:hover:bg-n-7"
        href={item.url}
        key={index}
      >
        <div className="relative md:w-[80px] md:h-[80px] flex justify-center items-center w-10 h-10 mr-6">
          <div
